...
}catch(c){d.duration=200;}}},_createButtons:function(){var e=this,g=this.options,d,c=g.backBtnText,f=g.nextBtnText;this._removeButtons();if(g.navButtons==="none"){return;
}if(!this.buttons){d=g.navButtons;this.buttons=a('<ul class="pager"/>');this.buttons.addClass("bwizard-buttons");if(c!=""){this.backBtn=a("<li class='previous'><a href='#'>"+c+"</a></li>").appendTo(this.buttons).bind({click:function(){e.back();
return false;}}).attr("role","button");}if(f!=""){this.nextBtn=a("<li class='next'><a href='#'>"+f+"</a>").appendTo(this.buttons).bind({click:function(){e.next();
return false;}}).attr("role","button");}this.buttons.appendTo(this.element);}},_removeButtons:function(){if(this.buttons){this.buttons.remove();this.buttons=b;
}},_pageLize:function(f){var d=this,e=this.options,h=/^#.+/;var g=false;this.list=this.element.children("ol,ul").eq(0);var c=this.list.length;if(this.list&&c===0){this.list=null;
}if(this.list){if(this.list.get(0).tagName.toLowerCase()==="ol"){g=true;}this.lis=a("li",this.list);this.lis.each(function(){a(this).find(".label").remove();
var j=a(this).index()+1;a(this).css("z-index",d.lis.length-j);a(this).contents().wrap('<span class="hidden-phone"/>');a(this).prepend('<span class="label">'+j+"</span>");
if(!g){a(this).find(".label").addClass("visible-phone");}});}if(f){this.panels=a("> div",this.element);this.panels.each(function(k,l){var j=a(l).attr("src");
if(j&&!h.test(j)){a.data(l,"load.bwizard",j.replace(/#.*$/,""));}});this.element.addClass("bwizard clearfix");if(this.list){this.list.addClass("bwizard-steps clearfix").attr("role","tablist").attr("role","tab");
...
},_refreshStep:function(){var c=this.options;if(this.lis){this.lis.removeClass("current").attr("aria-selected",false).find(".label").removeClass("badge-inverse");
if(c.activeIndex>=0&&c.activeIndex<=this.lis.length-1){if(this.lis){this.lis.eq(c.activeIndex).addClass("current").attr("aria-selected",true).find(".label").addClass("badge-inverse");
...
}if(this.lis){this.lis.removeClass("current").removeAttr("role");this.lis.each(function(){if(a.data(this,"destroy.bwizard")){a(this).remove();}else{a(this).removeAttr("aria-selected");
...
}}else{e.insertBefore(this.panels[d]);}if(this.list&&this.lis){h=a((f.stepHeaderTemplate||c._defaults.stepHeaderTemplate).replace(/#\{title\}/g,g));h.addClass("ui-widget-header ui-corner-all ui-priority-secondary").data("destroy.bwizard",true);
