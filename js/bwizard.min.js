...
if(this.list&&c===0){this.list=null;}if(this.list){if(this.list.get(0).tagName.toLowerCase()==="ol"){g=true;}this.lis=a("li",this.list);this.lis.each(function(){var j=a(this).index()+1;
if(e.clickableSteps){a(this).click(function(i){i.preventDefault();this.show(j);});a(this).contents().wrap('<a href="#step'+j+'" class="hidden-phone"/>');
}else{a(this).contents().wrap('<span class="hidden-phone"/>');}a(this).attr("role","tab");a(this).css("z-index",d.lis.length-j);a(this).prepend('<span class="label">'+j+"</span>");
...
if(j&&!h.test(j)){a.data(l,"load.bwizard",j.replace(/#.*$/,""));}});this.element.addClass("bwizard clearfix");if(this.list){this.list.addClass("bwizard-steps clearfix").attr("role","tablist");
